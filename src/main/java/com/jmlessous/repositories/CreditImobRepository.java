...
public interface CreditImobRepository extends CrudRepository<CreditImmobilier,Long> {
    ...
    @Query("SELECT c FROM CreditImmobilier c WHERE c.compteCredit.utilisateurC.idUser= :id")
    ...
    @Query("SELECT c FROM CreditImmobilier c WHERE c.STATUS='ENCOURSDETRAITEMENT'")
